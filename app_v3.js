...
function loadDemoData() {
    const isVi = appData.lang === "vi";
    const confirmMsg = isVi 
        ? "Bạn có chắc chắn muốn tải dữ liệu mẫu không? Hành động này sẽ ghi đè lên dữ liệu hiện tại."
        : "Are you sure you want to load the demo data? This will overwrite your current plan.";
        
    if (confirm(confirmMsg)) {
        saveState(isVi ? "Tải dữ liệu mẫu" : "Load demo data");
        
        const currentLang = appData.lang;
        const currentTheme = appData.themeSettings;
        
        appData = JSON.parse(JSON.stringify(DEFAULT_DATA));
        appData.lang = currentLang;
        appData.themeSettings = currentTheme;
